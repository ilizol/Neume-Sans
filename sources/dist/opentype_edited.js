...
(function (global, factory) {
	...
	Glyph.prototype.drawPoints = function(ctx, x, y, fontSize) {
	    function drawCircles(l, x, y, scale) {
	        ...
	        for (var j = 0; j < l.length; j += 1) {
	            ...
	            ctx.arc(x + (l[j].x * scale), y + (l[j].y * scale), fontSize / 36 * scale, PI_SQ, false);
	        }
	 ...
	 * @param  {number} [fontSize=72] - Font size in pixels. We scale the glyph units by `1 / unitsPerEm * fontSize`.
	 */
	Glyph.prototype.drawMetrics = function(ctx, x, y, fontSize, advanceWidth) {
	    var scale;
	    x = x !== undefined ? x : 0;
	    y = y !== undefined ? y : 0;
	    fontSize = fontSize !== undefined ? fontSize : 24;
	    ...
	    // Draw the advance width / 2
	    ctx.strokeStyle = 'gray';
	    draw.line(ctx, x + (advanceWidth/2 * scale), -10000, x + (advanceWidth/2 * scale), 10000);
	};
